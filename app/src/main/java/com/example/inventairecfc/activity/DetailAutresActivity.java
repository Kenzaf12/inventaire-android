...
public class DetailAutresActivity extends AppCompatActivity {

    private EditText etNumRef, etRefImmo, etCab, etNlocal,
            etDesignation, etUnite, etQte, etMontGl,
            etModele, etMarque, etFournisseur, etDescTech,
            etAffectation, etDateInvent, etHeureInvent, etAgent;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_autres);
        ...
        etNumRef = findViewById(R.id.etNumRef);
        etRefImmo = findViewById(R.id.etRefImmo);
        etCab = findViewById(R.id.etCab);
        etNlocal = findViewById(R.id.etNlocal);
        etDesignation = findViewById(R.id.etDesignation);
        etUnite = findViewById(R.id.etUnite);
        etQte = findViewById(R.id.etQte);
        etMontGl = findViewById(R.id.etMontGl);
        etModele = findViewById(R.id.etModele);
        etMarque = findViewById(R.id.etMarque);
        etFournisseur = findViewById(R.id.etFournisseur);
        etDescTech = findViewById(R.id.etDescTech);
        etAffectation = findViewById(R.id.etAffectation);
        ...
        // Use same etat values as backend InventAutres entity
        ...
        etNumRef.setText(getIntent().getStringExtra("numRef"));
        etRefImmo.setText(getIntent().getStringExtra("refImmo"));
        etCab.setText(getIntent().getStringExtra("cab"));
        etNlocal.setText(getIntent().getStringExtra("nlocal"));
        etDesignation.setText(getIntent().getStringExtra("designation"));
        etUnite.setText(getIntent().getStringExtra("unite"));
        etModele.setText(getIntent().getStringExtra("modele"));
        etMarque.setText(getIntent().getStringExtra("marque"));
        etFournisseur.setText(getIntent().getStringExtra("fournisseur"));
        etDescTech.setText(getIntent().getStringExtra("descTech"));
        etAffectation.setText(getIntent().getStringExtra("affectation"));

        double qte = getIntent().getDoubleExtra("qte", 0.0);
        etQte.setText(qte == 0.0 ? "" : String.valueOf(qte));
        double montGl = getIntent().getDoubleExtra("montGl", 0.0);
        etMontGl.setText(montGl == 0.0 ? "" : String.valueOf(montGl));
    ...
    private void saveAutres() {
        InventAutres item = new InventAutres();
        item.setId(itemId);
        item.setNumRef(etNumRef.getText().toString().trim());
        item.setRefImmo(etRefImmo.getText().toString().trim());
        item.setCab(etCab.getText().toString().trim());
        item.setNlocal(etNlocal.getText().toString().trim());
        item.setDesignation(etDesignation.getText().toString().trim());
        item.setUnite(etUnite.getText().toString().trim());
        item.setModele(etModele.getText().toString().trim());
        item.setMarque(etMarque.getText().toString().trim());
        item.setFournisseur(etFournisseur.getText().toString().trim());
        item.setDescTech(etDescTech.getText().toString().trim());
        item.setAffectation(etAffectation.getText().toString().trim());
        ...
        catch (NumberFormatException e) { item.setQte(0.0); }
        try { item.setMontGl(Double.parseDouble(etMontGl.getText().toString())); }
        catch (NumberFormatException e) { item.setMontGl(0.0); }
    ...
    private void uploadPhotoThenSave(InventAutres item) {
        try {
    ...
    private void saveToBackend(InventAutres item) {
        String token = "Bearer " + sessionManager.getToken();
        Callback<InventAutres> cb = new Callback<InventAutres>() {
            @Override public void onResponse(Call<InventAutres> call, Response<InventAutres> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(DetailAutresActivity.this, "Sauvegardé ✓", Toast.LENGTH_SHORT).show();
                    finish();
                } else {
                    Toast.makeText(DetailAutresActivity.this, "Erreur de sauvegarde", Toast.LENGTH_SHORT).show();
                }
            }
            @Override public void onFailure(Call<InventAutres> call, Throwable t) {
                Toast.makeText(DetailAutresActivity.this, "Erreur de connexion", Toast.LENGTH_SHORT).show();
            }
        };
        if (itemId == null || itemId == 0) {
            apiService.createAutres(token, item).enqueue(cb);
        } else {
            apiService.updateAutres(token, itemId, item).enqueue(cb);
        }
    }
    ...
    private void confirmDelete() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Supprimer", (d, w) -> deleteAutres())
                .setNegativeButton("Annuler", null)
                .show();
    }

    private void deleteAutres() {
        String token = "Bearer " + sessionManager.getToken();
        apiService.deleteAutres(token, itemId).enqueue(new Callback<Void>() {
            @Override public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(DetailAutresActivity.this, "Supprimé", Toast.LENGTH_SHORT).show();
                    finish();
                } else {
                    Toast.makeText(DetailAutresActivity.this, "Erreur de suppression", Toast.LENGTH_SHORT).show();
                }
            }
